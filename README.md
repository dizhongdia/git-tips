...
2. 通过ssh-keygen 程序生成
```
   ssh-keygen -t rsa -C "87xxxxx@qq.com"
```
...
git push -u
```
弹出登录框后，输入用户名和密码（**密码是刚刚复制的token**）。

输入代码前后，.gitconfig配置文件的变化如下：

![image](https://user-images.githubusercontent.com/46380119/124350412-38389680-dc27-11eb-8d1c-63a55cb35330.png)
...
tokens在本地的文件名：.git-credentials，一般位于C:->Users->用户名->.git-credentials



### Git的作用：

分布式版本管理，有其他的版本管理工具，但是Git比较先进。所谓的分布式就是没有“中央服务器”，每个人的电脑都是一个完整的版本库。
...
```java
fatal: unable to access 'https://github.com/youafaw/test.git/': OpenSSL SSL_connect: Connection was reset in connection to github.com:443
```
...
示例：
git commit -m "[feat]实现了美图的功能"	
```
本地版本库和远程版本库建立连接，把远程版本库命名为origin，默认是origin，当然你可以命名为其他，但不建议：
```
语法：
git remote add origin [url]
示例：
git remote add origin  https://github.com/youafaw/test.git/
```
把本地版本库推入远程库：
```
...
### github上的脑图
很喜欢这个格式，可以包含非常多的信息，以前我自己用脑图使用默认的格式经常是一个个的格子，放的信息少，而且还难看。
这个脑图个人切片并标注了，原始版来源见：https://github.com/521xueweihan/git-tips/blob/master/assets/git.png
